...
# row: features
# column: Nuclei
mydata <- read.csv("/home/j.fermin/EEL5934-cell-map/features_mean_SD_0105_new.csv", header = TRUE)

# exclude first column
mydata <- mydata[-1,]


myseurat <- CreateSeuratObject(counts = mydata, project='cell-mapping', min.cells = 1, min.features=5)
...
# apply PCA
myseurat <- RunPCA(myseurat, features = VariableFeatures(object = myseurat),npcs =10)
...
# Perform UMAP
myseurat <- RunUMAP(myseurat, dims = 1:10, n.components = 2L, min.dist = 0.001, metric='cosine')
...
myseurat.markers <- FindAllMarkers(myseurat)
...
write.csv(umap_data, file = "/home/j.fermin/EEL5934-cell-map/umap__mean_SD_reg_new.csv", row.names = FALSE)
